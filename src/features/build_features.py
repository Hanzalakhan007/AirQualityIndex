...
def _build_feature_dataframe(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    dataframe["pm25_rolling_24"] = dataframe["pm2_5"].astype(float).rolling(window=24).mean()
    dataframe["pm10_rolling_24"] = dataframe["pm10"].astype(float).rolling(window=24).mean()
